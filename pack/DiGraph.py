    ...
    def __eq__(self, other):
        ...
        for node1, node2 in zip(self.get_all_v().values(), other.get_all_v().values()):
            ...
            if len(self.all_out_edges_of_node(node1.id)) != len(other.all_out_edges_of_node(node2.id)):
                return False
            for in1, in2 in zip(self.all_out_edges_of_node(node1.id).keys(),
                                other.all_out_edges_of_node(node2.id).keys()):
                if in1 != in2:
                    return False
            if len(self.all_in_edges_of_node(node1.id)) != len(other.all_in_edges_of_node(node2.id)):
                return False
            for out1, out2 in zip(self.all_in_edges_of_node(node1.id).keys(),
                                  other.all_in_edges_of_node(node2.id).keys()):
                if out1 != out2:
                    return False
